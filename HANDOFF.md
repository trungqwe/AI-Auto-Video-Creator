...
- Không tự phát minh cạnh Stage Run: `CT-STATE-008` không định nghĩa lối ra từ `WAITING_*`/`FAILED_RETRYABLE`; `OUTCOME_UNKNOWN` chỉ reconcile. Mapper waiting chỉ dùng `wait_reason` structured của `CT-API-007`, không timestamp/progress/UI. Target reconcile của Operation `OUTCOME_UNKNOWN` chưa được contract liệt kê, là open item phải được independent audit đóng trước RED oracle tương ứng.
