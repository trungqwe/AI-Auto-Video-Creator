...
- Correction replay: receipt accepted được materialize bằng `INSERT ... RETURNING`; same-key/same-hash replay đọc `receipt_id`, `command_id`, `operation_id`, `resource_ref`, `accepted_at` và `current_revision` từ durable row, chỉ trả transient `disposition="duplicate"`. Raw correction RED: `correction-red-replay-receipt-stdout.txt`.
- Closure evidence run `run-m2-p2-20260914074301`: P2 11/11, P1 11/11, P0 33/33, M1 93/93 (0 skipped); Python 3.13.15, psycopg 3.3.5, psycopg-pool 3.3.1, PostgreSQL 18.6, `CREATEDB=true`, pool thật `psycopg_pool.ConnectionPool`, orphan=0, secret scan CLEAN. P2-aware `synthesizer_p2.py --verify-only` PASS và SHA-256 DAG hợp lệ.
