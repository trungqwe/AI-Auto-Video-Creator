...
| M2 | 🟡 `M2-P2_PLAN_READY_FOR_REVIEW` | P1 đã `ACCEPTED / CLOSED`; SPEC/plan P2 đã khóa contract scope, production `0002`, canonical idempotency, exact 11 RED oracle và 6 gates. Chờ independent review plan; không viết Behavioral RED hay implementation P2. |
...
- **M2-P2 checkpoint hiện hành**: `M2-P2_PLAN_READY_FOR_REVIEW`. Quyền P2 là `SPEC → PLAN → RED`, nhưng checkpoint này dừng để independent review plan; không tạo Behavioral RED hoặc implementation P2. Implementation chỉ được xét sau RED hợp lệ, raw stdout/evidence và independent audit. M3 và Phân hệ A tiếp tục `NOT AUTHORIZED`.
