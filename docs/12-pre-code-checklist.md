...
| M2 | 🟡 `M2-P1_READY_FOR_REVIEW` | P1 đã chạy GREEN 11/11 trên PostgreSQL disposable thật; frozen P0 33/33, M1 93/93, 6/6 gate PASS; chờ independent audit, P2 vẫn khóa |
...
- **M2-P1 implementation/evidence checkpoint hiện hành**: 11/11 mandatory oracle GREEN trên PostgreSQL 18.6 thật với disposable DB function-scoped; frozen P0 exact set 33/33, M1 93/93, secret scan 0 findings, provenance và SHA-256 DAG hợp lệ. P1-aware `synthesizer_p1.py --verify-only` trả `VALIDATION: PASS`; phase dừng tại `M2-P1_READY_FOR_REVIEW` để independent audit. Không mở P2.
