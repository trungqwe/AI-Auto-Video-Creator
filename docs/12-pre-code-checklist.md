...
| M2 | 🟡 `M2-P2_BEHAVIORAL_RED_BLOCKED_EXTERNAL` | Harness đã encode 11 behavioral oracle và P2-001/002 RED hợp lệ; run prerequisite xác nhận `M2_TEST_PG_DSN` absent nên P2-003..011 cùng PostgreSQL/CREATEDB/orphan chưa thể chạy. Implementation vẫn khóa. |
...
- **M2-P2 checkpoint hiện hành**: `M2-P2_BEHAVIORAL_RED_BLOCKED_EXTERNAL`. Harness đã encode behavior P2-001..011 và P2-001/002 có RED đúng structural capability. Run prerequisite ghi `M2_TEST_PG_DSN=ABSENT`; P2-003..011 chưa được chạy/simulate. Cần PostgreSQL 18.6 thật với `CREATEDB`, raw full exact-11 stdout và orphan=0 trước independent audit. M3 và Phân hệ A tiếp tục `NOT AUTHORIZED`.
