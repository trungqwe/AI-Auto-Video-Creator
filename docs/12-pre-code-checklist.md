...
| M2 | 🟡 `IMPLEMENTATION IN PROGRESS (M2-P0 ACCEPTED / CLOSED, M2-P1 Plan ACCEPTED, Behavioral RED BLOCKED_EXTERNAL)` | User đã ủy quyền RED; 11 oracle đã collect/importable, có fixture DB function-scoped/raw evidence và bootstrap test-only tách downstream oracle khỏi migration stub, nhưng thiếu `M2_TEST_PG_DSN` |
...
- **Correction sau audit `42e1859b19460f8254d8d5be500f910a1c262570`**: P1-005/006/007/011 có bootstrap schema test-only trực tiếp trong disposable DB, nên khi PostgreSQL sẵn sàng sẽ fail đúng UoW/composite-FK/workspace-scoping/pool oracle thay vì `MigrationRunner`. Không production behavior. Thiếu `M2_TEST_PG_DSN` nên chưa oracle nào được tính vào confirmation hiện hành; phase dừng tại `M2-P1_BEHAVIORAL_RED_BLOCKED_EXTERNAL`. Không sửa implementation P0, không mở P2; M3 và Phân hệ A tiếp tục bị khóa hoàn toàn (`NOT AUTHORIZED`).
