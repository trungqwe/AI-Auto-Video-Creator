...
**Trạng thái:** `M2-P1_ACCEPTED_CLOSED; M2-P2_PLAN_READY_FOR_RED_APPROVAL` (Correction audit đã khóa chi tiết SPEC/plan P2 trước Behavioral RED.)
**Ngày lập:** 13-09-2026 (User đã chấp thuận plan sau independent re-audit HEAD `5ba3a1601f0e1402e54a82feb5b44fe94cda9197`.)
**Điểm dừng bắt buộc hiện hành:** `M2-P2_PLAN_READY_FOR_RED_APPROVAL`. M2-P1 là `ACCEPTED / CLOSED`; SPEC/plan P2 đã khóa requirement scope, schema, byte-canonicalization, production CAS, oracle, gates và evidence protocol. Chờ independent review/approval plan; không viết Behavioral RED hay implementation P2 trong checkpoint này. Sau RED hợp lệ, raw evidence và independent audit mới được xét quyền implementation. Không sửa P0; M3 và Phân hệ A vẫn `NOT AUTHORIZED`.
**Căn cứ:**
...
- **Mục tiêu**:
  ...
  4. Khóa identity hash bằng **RFC 8785 JSON Canonicalization Scheme (JCS)** duy nhất: input parse thành I-JSON, serialize JCS bytes UTF-8, object keys theo Unicode code-point order, string escaping/Unicode theo ECMAScript/JCS (Unicode giữ nguyên, không normalization), array giữ nguyên order, `null`/boolean theo literal JSON, số theo ECMAScript IEEE-754 JCS representation. NaN/Infinity bị reject; negative zero canonicalize thành `0`. Logical object gồm `workspace_id`, `command_name`, `payload` canonical, `expected_revision` khi command mutation revisioned và `policy_revision_id` khi có. Loại trừ `message_id`, `command_id`, correlation/causation IDs và timestamps; SHA-256 chạy trên exact JCS UTF-8 bytes, không hash raw JSON.
...
| `test_tst_m2_p2_004_same_key_same_canonical_payload_replays_same_receipt` | `CT-CMN-011`, `ADR-0004` | Fixed JCS vectors (key/whitespace/nested/array/Unicode/numeric supported domain) hash khác, hoặc replay không trả same persisted receipt với `duplicate` transient. |
