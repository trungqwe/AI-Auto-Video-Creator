...
- **Mục tiêu**:
  1. Hiện thực hóa Raw SQL Migration Runner native:
     ...
     - **Disposable Test Database Protocol**: Kiểm thử chu trình rollback full down/up và integration tests **bắt buộc chạy trên disposable test database** độc lập. Fixture `disposable_db` có function scope: mỗi mandatory behavioral oracle tạo database `m2_p1_test_<uuid>` riêng, không tái sử dụng migration/applied state từ oracle trước.
       ...
       * **RED bootstrap hẹp cho downstream oracle**: Chỉ P1-005, P1-006, P1-007 và P1-011 được dùng fixture DDL test-only tạo object prerequisite trực tiếp trong disposable database function-scoped; fixture không gọi `MigrationRunner.migrate_up()`, không tạo/sửa production migration, và bị teardown cùng database. P1-006 cố ý không tạo composite FK `(workspace_id, actor_id)` để RED được quan sát bằng `DID NOT RAISE ForeignKeyViolation`; đây không phải production schema behavior.
...
- **RED Oracle**:
  ...
  - `test_tst_m2_p1_006_workspace_isolation_and_composite_fk_enforcement`: Bootstrap test-only cố ý thiếu composite FK, seed Workspace A/B + Actor A, rồi insert AuthSession (Workspace B, Actor A) phải ném `ForeignKeyViolation` nhưng không ném -> RED `DID NOT RAISE ForeignKeyViolation`, chứng minh đúng invariant thiếu.
