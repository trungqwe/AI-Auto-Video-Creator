...
**Trạng thái:** `M2-P7B_AUTHORITY_READY_FOR_REVIEW`; `M2-P7B_BEHAVIORAL_RED_LOCKED`; `M2-P7B_IMPLEMENTATION_LOCKED`. Đây là thiết kế đề xuất để independent review, không phải quyền tạo RED/code. Phụ thuộc P7A `ACCEPTED_CLOSED`, `CT-API-008`, hợp đồng `09-contracts/01-control-api-and-stream.md` và ADR-0007. Không có migration `0008`, Redis, broker, Temporal hoặc WebSocket trong phạm vi này.
...
`src/controlplane/application/projections/**` bị **rút khỏi Allowed Scope**. `application/projections/__init__.py`, `operation_view_mapper.py`, P3 projection writer `infrastructure/db/projections/postgres.py`, migrations `0001..0007`, P7A source/test/oracle/evidence, `src/controlplane/ui/**`, `src/m1proof/**`, P8/P9/M3/Phân hệ A đều read-only/forbidden. Nếu cần sửa accepted P3 writer hoặc migration, STOP và xin independent review; không tự nới scope.
...
**Cursor contract đề xuất trước RED.** Cursor là chuỗi ASCII thập phân canonical của `stream_event_id` dương, trong miền signed BIGINT `1..9223372036854775807`; không dấu, khoảng trắng, số 0 đầu hoặc dạng khác. Không có query `cursor` và không có `Last-Event-ID`: bắt đầu tại high-water mark **đã commit và nhìn thấy của workspace tại lúc subscribe**, chỉ phát sự kiện mới hơn; không tự replay toàn bộ lịch sử. Reconnect nhận query `cursor` hoặc `Last-Event-ID`; nếu cả hai hiện diện phải bằng nhau, nếu khác trả `400 CURSOR_MISMATCH`. Cursor đã nhận được replay **exclusive** (`stream_event_id > cursor`), `ORDER BY stream_event_id ASC`; `id:` bằng chính cursor, không dùng wall-clock. Invalid syntax/range → `400 INVALID_CURSOR` JSON ProblemDetail, trước khi mở SSE. Cursor lớn hơn max đã nhìn thấy của workspace → `409 CURSOR_AHEAD`; cursor không thuộc workspace nhưng nằm trong miền hiện tại → `409 UNKNOWN_CURSOR` cùng thông tin an toàn, không tiết lộ foreign row. Không thay cursor của workspace này bằng cursor của workspace khác. Khoảng trống sequence không tự là dấu retention hoặc mất event.

**Retention/watermark.** Schema P3 hiện có `cp_operation_stream_retention_watermarks(workspace_id, minimum_available_cursor, minimum_available_at)` và `cp_operation_stream.stream_event_id`; đủ cột để **đọc** watermark, không cần `0008`. `minimum_available_cursor` được hiểu là lower bound giữ lại cho workspace, chỉ được advance atomically cùng retention deletion bởi một authority riêng; P7B không tạo retention writer. Khi chưa có watermark row, xem như chưa kích hoạt retention (không suy hết hạn từ `MIN(stream_event_id)`); nếu có row và requested cursor `< minimum_available_cursor`, phát một resync signal rồi đóng kết nối, không nhảy ngầm tới hiện tại. Việc xóa row mà không cập nhật watermark là invariant violation và phải fail closed; chính sách retention/writer tương lai cần checkpoint riêng. First subscribe không cursor không bị coi là expired.
...
**Connection và backpressure.** Đề xuất bounded polling mỗi 1 giây, `LIMIT 100` mỗi query, không LISTEN/NOTIFY hay in-memory event bus làm acceptance path. Mỗi poll borrow P1 connection trong phạm vi query ngắn rồi release trước khi `yield`; sync DB query không block event loop (worker thread có giới hạn), tối đa 16 SSE clients và tối đa 3 DB polls đồng thời để giữ chỗ cho P7A pool mặc định 4. Client chậm dừng prefetch, chỉ giữ một page tối đa 100 items, không queue không giới hạn; nếu vượt khả năng gửi thì đóng, client reconnect bằng cursor cuối nhận. Disconnect/cancel dừng polling, giải phóng page và connection; DB statement có timeout hữu hạn, không thread/connection leak. Adapter chỉ nhận connection, không sở hữu pool/commit/rollback; P1 manager sở hữu lifecycle. Schema hiện chỉ có PK trên `stream_event_id`, chưa có `(workspace_id, stream_event_id)` index: future GREEN phải đo `EXPLAIN`/bounded read trên dataset đa workspace; nếu cần index/migration thì STOP xin authority delta, không tạo `0008` âm thầm.

**Open STOP gate — commit-order của `BIGSERIAL`.** `stream_event_id BIGSERIAL` là thứ tự cấp sequence, **không** bảo đảm thứ tự commit: transaction A có thể nhận N, B nhận N+1 và commit trước; stream gửi N+1 rồi A commit khiến query `> N+1` bỏ mất N. PostgreSQL cũng không rollback `nextval` nên không thể coi lỗ ID là transaction còn chờ ([PostgreSQL sequence documentation](https://www.postgresql.org/docs/18/functions-sequence.html)). Đây là suy luận từ schema/writer P3 hiện tại, chưa phải lỗi được chứng minh bằng RED P7B. Read-only polling không thể chứng minh no-loss dưới concurrent writers theo contract. Ba hướng đã đánh giá (điểm 1–10; trọng số correctness ×3, integration risk ×2, sáu tiêu chí còn lại ×1; cao là tốt):
...
| B — serialize P3 append trước khi cấp cursor tới commit | 9 / 5 / 6 / 5 / 8 / 7 / 7 / 6 | 76 | Ưu tiên xem xét: giữ schema, nhưng đòi authority sửa accepted P3 writer và chứng minh mọi writer đều dùng lock. |
...
Điểm số không ghi đè hard gate correctness: A bị loại dù điểm tổng cao; B là đề xuất cần independent review/authority delta, **chưa được cấp phép**. Không tạo RED/code cho tới khi khóa cơ chế ordering an toàn và chứng minh bằng concurrent transaction test. Nếu reviewer chọn B/C, sửa scope và oracle trong docs-only checkpoint mới trước khi mở RED. Không sửa P3 source trong checkpoint này.

**Future exact Behavioral RED identities, chưa được tạo:**

| Identity | Oracle bắt buộc | Expected behavioral RED seam sau structural wiring tối thiểu |
|---|---|---|
| `test_tst_m2_p7b_001_sse_stream_format_and_headers` | Authenticated P7A route trả SSE header + đúng event frame N từ durable row. | `SseStreamPresenter.frames` chưa implement: ứng dụng thật trả safe 500, không phải setup/import/DSN failure; không cho 404 do route chưa mount làm RED giả. |
...
Workspace isolation cần RED identity riêng vì adapter SQL mới có thể rò dữ liệu dù P7A session đã đúng. Disconnect cleanup là GREEN hardening độc lập vì cần generator/connection thực, không gộp vào ba oracle hành vi đầu. Structural seams chỉ được tạo **sau** quyền RED riêng; RED phải fail đúng seam, không được là 404 chưa mount, import, DB hay DSN failure.

**Future independent GREEN hardening catalogue** (mỗi ID là testcase/probe/evidence riêng, không alias aggregate): H01 actual route mounted; H02 `text/event-stream` headers; H03 UTF-8 event framing; H04 authenticated workspace binding; H05 foreign workspace exclusion; H06 monotonic cursor dưới ordering mechanism đã duyệt; H07 reconnect replay; H08 expired cursor → exact resync frame/close; H09 malformed cursor; H10 query/header mismatch; H11 future cursor; H12 unknown/foreign cursor; H13 GET-only/no mutation; H14 safe allowlist payload; H15 redaction không rò stack/token/secret; H16 concurrent clients; H17 abrupt disconnect cleanup; H18 bounded `LIMIT 100` và query plan; H19 không leak thread; H20 không leak connection; H21 actual P7A HTTPS; H22 Host validation; H23 Origin validation; H24 session expiry/revocation trong long-lived stream; H25 accepted P7A oracle 4/4; H26 accepted P7A hardening 36/36; H27 P6 4/4; H28 P5B 5/5; H29 P5A 5/5; H30 P4 9/9; H31 P3 11/11; H32 P2 11/11; H33 P1 11/11; H34 P0 33/33; H35 architecture 6/6; H36 M1 93/93; H37 concurrent two-writer commit-inversion proof sau khi ordering authority được duyệt. H25–H36 phải chạy suite thật và lưu JUnit riêng, không dùng boolean alias.

**Future evidence protocol.** P7B cần semantic profile/run **mới**, immutable source SHA và exact oracle identities/count, runtime/static, migration=no-change, secret scan, hash DAG, provenance/tamper-negative, zero fail/error/skip và regressions. `commands.jsonl` chỉ ghi `argv`, exit code và artifact của external command thực sự được chạy; policy/skip ghi record riêng `executed=false` + lý do, không bịa argv. Không ghi DSN/token; không sửa P7A GREEN đã accepted hoặc GREEN cũ bị từ chối. RED không được coi hợp lệ nếu failure là setup/import/DSN/route chưa mount. PASS claim SSE chỉ sau GREEN + independent review; hiện tại **cấm** claim P7B bắt đầu hoặc hoàn tất.
