...
**Điểm dừng bắt buộc:** `M2-P2_RED_HARNESS_FINAL_READY_BLOCKED_EXTERNAL`. P2 plan/final harness đã ACCEPTED ở cấp collection, nhưng `M2_TEST_PG_DSN` chưa có; không thể chạy RED PostgreSQL thật hoặc kiểm orphan. Implementation chỉ có thể được xét sau RED hợp lệ, evidence và independent audit. M3 hoặc Phân hệ A vẫn `NOT AUTHORIZED` cho tới khi M2 đạt exit gate và có User Checkpoint riêng.
**Căn cứ kiến trúc:**
