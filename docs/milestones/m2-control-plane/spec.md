...
**Điểm dừng bắt buộc:** P1..P7A là `ACCEPTED / CLOSED`. P7A source/tooling `6c3a52bde905ee5e71e12334da1873ed20f5c5db` và GREEN `run-m2-p7a-green-20260916224033` đã accepted. P7B authority accepted tại `763aa63b`; RED cũ `run-m2-p7b-red-20260917000606` bị review từ chối do #001 sai first-subscribe baseline và giữ bất biến. Corrected Behavioral RED `run-m2-p7b-red-20260917002633` đang chờ independent review, chưa `ACCEPTED`. Future fence/sequence allocation/INSERT phải nằm trong **một SQL statement** tương thích autocommit/P1 UoW. P7B implementation vẫn khóa; không sửa P3 hay migration `0008`. P8/P9 khóa; M3 và Phân hệ A `NOT AUTHORIZED`.
**Căn cứ kiến trúc:**
...
P5B phải hỗ trợ `VERIFIED → MISSING` khi verify sau phát hiện mất; không thêm recovery từ `MISSING`, `CORRUPT` hay `OUTCOME_UNKNOWN`. Full graph vẫn là `VERIFIED → CLEANUP_ELIGIBLE → CLEANUP_AUTHORIZED → DELETED`, nhưng P5B chỉ được thực thi đến `CLEANUP_AUTHORIZED`. Authorization là immutable fact không mutable `status`, giữ version/hash/evidence/policy/epoch/owner/audit refs; edge cuối chỉ do package sau commit sau bằng chứng delete thật. P5B không thực thi/simulate delete, không persist `DELETED`, không phát `CleanupCompleted`, upload, Drive lifecycle, local journal hoặc claim external byte integrity.
...
**P7A design lock sau RED acceptance:** [implementation plan P7A](./implementation-plan.md#m2-p7a-module-h--control-api-core-local-https--security-boundaries) là authority cho exact future file scope, application ports/PostgreSQL adapters, migration `0007` (kể cả `cp_auth_sessions.token_hash` và technical-detail access audit), `create_app` middleware order, Host/Origin/session/CSRF policy, actual Uvicorn+FastAPI TLS proof và **36 independent hardening keys H01–H36**. HTTP mutation dùng đúng một caller-owned P1 UoW cho P2 lock/hash/receipt/idempotency, P5A hoặc StartBatch business mutation và P3 outbox; không gọi `IdempotencyCoordinator.submit()` vì nó tự mở UoW khác. P7A `ControlApiCommandService` không SQL/Infrastructure import; composition adapter bind accepted P2/P3/P5A adapters cùng connection, không tự commit/rollback. StartBatch receipt có non-null `operation_id`, batch `resource_ref`, `current_revision=NULL` vì P6 batch chưa có revision; outbox `production_batch.start_accepted` chỉ là durable dispatch intent, không phải Temporal completion. API facade không sở hữu SQL; `api/security.py`, `api/technical_details.py`, `api/tls.py` giữ import/call shape của accepted oracle byte-exact. Không tạo migration, code production hoặc GREEN claim trong checkpoint tài liệu này.
