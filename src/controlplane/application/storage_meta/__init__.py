...
from controlplane.domain.storage_meta import (
    ArtifactLocation,
    ArtifactVersion,
...
_UNSAFE_LOCATOR = re.compile(
    r"(?:^[/\\]|^[A-Za-z]:[\\/]|(?:^|[/\\])\.\.(?:[/\\]|$)|^file://|[?&](?:signature|token|credential)=|[;&|`])",
    re.IGNORECASE,
)
...
def _location(row: Any) -> ArtifactLocation | None:
    ...
    return ArtifactLocation(
        ...
        row[9],
        row[10],
        ArtifactLocationState(str(row[11])),
        int(row[12]),
        _iso(row[13]),
        _iso(row[14]),
    )
...
class ArtifactVersionStore:
    def register(self, **kwargs: Any) -> ArtifactVersion:
        ...
        existing = _version(
            connection.execute(
                f"SELECT {_VERSION_COLUMNS} FROM controlplane.cp_artifact_versions WHERE workspace_id=%s AND artifact_id=%s AND sha256_hash=%s",
                (kwargs["workspace_id"], kwargs["artifact_id"], kwargs["sha256_hash"]),
            ).fetchone()
        )
        ...
        row = connection.execute(
            "INSERT INTO controlplane.cp_artifact_versions (artifact_version_id, workspace_id, artifact_id, sha256_hash, size_bytes, mime_type, artifact_kind, owner_ref, lineage_ref, retention_ref, sensitivity_ref) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (workspace_id, artifact_id, sha256_hash) DO NOTHING RETURNING "
            + _VERSION_COLUMNS,
            (
                version_id,
                kwargs["workspace_id"],
                ...
                kwargs["sha256_hash"],
                kwargs["size_bytes"],
                kwargs["mime_type"].strip(),
                kwargs["artifact_kind"],
                kwargs["owner_ref"],
                kwargs.get("lineage_ref"),
                kwargs["retention_ref"],
                kwargs["sensitivity_ref"],
            ),
        ).fetchone()
        if row is None:
            row = connection.execute(
    ...
    def get_scoped(self, **kwargs: Any) -> ArtifactVersion | None:
        ...
        return _version(
            connection.execute(
                f"SELECT {_VERSION_COLUMNS} FROM controlplane.cp_artifact_versions WHERE workspace_id=%s AND artifact_version_id=%s",
                (kwargs["workspace_id"], kwargs["artifact_version_id"]),
            ).fetchone()
        )
...
class ArtifactLocationStore:
    def declare(self, **kwargs: Any) -> ArtifactLocation:
        ...
        if not locator or _UNSAFE_LOCATOR.search(locator):
            raise ValueError("VALIDATION_ERROR: unsafe logical locator")
        location_id = str(uuid.uuid4())
        row = connection.execute(
            "INSERT INTO controlplane.cp_artifact_locations (location_id, workspace_id, artifact_version_id, backend_ref, provider_namespace_ref, provider_object_ref, logical_locator, state, revision) VALUES (%s,%s,%s,%s,%s,%s,%s,'DECLARED',1) RETURNING "
            + _LOCATION_COLUMNS,
            (
                location_id,
                kwargs["workspace_id"],
                ...
                kwargs["backend_ref"],
                kwargs["provider_namespace_ref"],
                kwargs["provider_object_ref"],
                locator,
            ),
        ).fetchone()
    ...
    def get_scoped(self, **kwargs: Any) -> ArtifactLocation | None:
        ...
        return _location(
            connection.execute(
                f"SELECT {_LOCATION_COLUMNS} FROM controlplane.cp_artifact_locations WHERE workspace_id=%s AND location_id=%s",
                (kwargs["workspace_id"], kwargs["location_id"]),
            ).fetchone()
        )
    ...
    def transition(self, **kwargs: Any) -> ArtifactLocation:
        ...
        if requested is ArtifactLocationState.DELETED:
        ...
        row = connection.execute(
            "UPDATE controlplane.cp_artifact_locations SET state=%s, revision=revision+1, verification_evidence_ref=COALESCE(%s, verification_evidence_ref), updated_at=CURRENT_TIMESTAMP WHERE workspace_id=%s AND location_id=%s AND revision=%s AND state=%s RETURNING "
            + _LOCATION_COLUMNS,
            (
                requested.value,
                evidence,
        ...
        result = _location(row)
        if result is None:
            latest = self.get_scoped(
                connection=connection,
                workspace_id=current.workspace_id,
                location_id=current.location_id,
            )
            raise RevisionConflictError(
                latest.revision if latest is not None else current.revision
            )
        return result

    def validate_transition(self, **kwargs: Any) -> object:
        if (
            ArtifactLocationState(kwargs["requested_state"])
            is ArtifactLocationState.DELETED
        ):
            raise ForbiddenTransitionError(
                aggregate_type="artifact_location",
                current_state=kwargs["current_state"],
                requested_next_state=kwargs["requested_state"],
...
class CleanupAuthorizationStore:
    def authorize(self, **kwargs: Any) -> CleanupAuthorization:
        ...
        if connection is None:
            raise TypeError("connection is required")
        location = ArtifactLocationStore().get_scoped(
            connection=connection,
            workspace_id=kwargs["workspace_id"],
            location_id=kwargs["location_id"],
        )
        if (
            location is None
            or location.state is not ArtifactLocationState.CLEANUP_ELIGIBLE
        ):
        ...
        row = connection.execute(
            ...
            (
                ...
                kwargs["reason"],
                kwargs["policy_revision"],
                kwargs["completion_evidence_ref"],
                kwargs["verification_evidence_ref"],
                kwargs["issued_at"],
                kwargs["expires_at"],
                kwargs["recovery_epoch"],
                kwargs["authorizing_owner_ref"],
                kwargs["actor_ref"],
                kwargs["audit_ref"],
                kwargs["correlation_ref"],
            ),
        ).fetchone()
        updated = connection.execute(
            "UPDATE controlplane.cp_artifact_locations SET state='CLEANUP_AUTHORIZED', revision=revision+1, updated_at=CURRENT_TIMESTAMP WHERE workspace_id=%s AND location_id=%s AND revision=%s AND state='CLEANUP_ELIGIBLE'",
            (
                kwargs["workspace_id"],
                kwargs["location_id"],
                kwargs["expected_revision"],
