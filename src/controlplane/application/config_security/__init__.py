...
class ConfigRevisionService(_ConfigRevisionOrchestrator):
    ...
    def create_revision(self, **kwargs: Any) -> ConfigRevision:
        ...
        config_revision_id = str(params.pop("config_revision_id", None) or uuid.uuid4())
        return self._repository_factory(connection).create_revision(**params, config_revision_id=config_revision_id, content_hash=request_hash(params["payload"]))
...
class SecretHandleStore:
    ...
    def register(self, **kwargs: Any) -> SecretHandle:
        ...
        missing = [name for name in required if name not in params]
        if missing: raise TypeError(f"missing required arguments: {', '.join(missing)}")
        secret_handle_id = str(params.pop("secret_handle_id", None) or uuid.uuid4())
        return self._repository_factory(connection).register_secret_handle(**params, secret_handle_id=secret_handle_id)
